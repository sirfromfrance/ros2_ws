...
class OpticalFlowNode(Node):
    def __init__(self):
        ...
        self.feature_params = dict(maxCorners=100, qualityLevel=0.01, minDistance=7, blockSize=7)
    ...
    def frame_processing(self, msg):
        ...
        try:
            ...
            if not self.optical_flow_init or self.prev_features is None or len(self.prev_features) < 15:
                ...
                if features is not None:
                    ...
                    for i, (new, old) in enumerate(zip(good_new, good_old)):
                        a, b = new.ravel()
                        c, d = old.ravel()
                        color_idx = i % len(self.color) 
                        cv_image = cv2.line(cv_image, (int(a), int(b)), (int(c), int(d)), self.color[color_idx].tolist(), 2)
                        cv_image = cv2.circle(cv_image, (int(a), int(b)), 5, self.color[color_idx].tolist(), -1)
